...
class StockContract : Contract {
    ...
    private fun verifyCreate(tx: LedgerTransaction) {
        requireThat {
            "Cannot create new StockState if one already exists." using tx.inputsOfType<StockState>().isEmpty()
        }
    }
    ...
    private fun verifyAddProduct(tx: LedgerTransaction) {
        ...
        requireThat {
            "Trying to update StockState with the same value." using (input != output)
        }
    }
